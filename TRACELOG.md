...
- Prompt/task: Companions need to attack creepers.
- Steps: Found creepers were suppressed twice and independently. `ModConfig.migrateAlertExclusions` force-injected `minecraft:creeper` into `alert.excludedMobs` on first load, so `AlertGoal` refused to target them, and `AvoidCreeperGoal` separately made companions flee on sight. Inverted the migration: added `AlertExclusionDefaults.withoutInjectedCreeper` and a second `creeperExclusionRemovedMigrated` marker so the injected entry is removed exactly once while every other exclusion, and any deliberate later re-add, is preserved. Deleted `AvoidCreeperGoal` and its registration. Added `CompanionCreeperPolicy` (ENGAGE default, RANGED_ONLY, AVOID) under the alert config block, and repointed `creeperWarning` to govern only the voice callout. Added the pure `CreeperEngagementRules` covering blast radius including charged creepers, the policy gate, melee back-off while a fuse burns, owner body-blocking, and owner interception priority, with `CreeperEngagementRulesTest` and a registered `creeperEngagementCheck` task. Added `CreeperTacticsGoal`, which holds only the MOVE flag so the class attack goal keeps attacking while this goal owns positioning: it interposes between a swelling creeper and the owner, backs away facing the creeper while the fuse burns, and closes again once it stops swelling. Extended `AlertConfigDefaultsTest` to cover both migration directions and their round trip.
- Rationale: Removing one suppression would not have been enough, since the config exclusion and the flee goal each independently prevented fighting creepers. Spacing rather than fleeing is the distinction that matters: the companion stays oriented on the creeper and re-engages the moment the fuse stops, which reads as competence rather than cowardice and matches how a player fights one. Body-blocking for the owner is deliberately the highest-priority creeper response, because the owner taking a blast costs far more than the companion taking it. Splitting positioning (MOVE) from attacking means no class weapon goal had to be rewritten.
- Build/Test: `gradlew check build` cannot run here; `maven.neoforged.net`, `maven.blamejared.com`, and `maven.theillusivec4.top` all return HTTP 403 on CONNECT so `neoform-runtime` never resolves. Compiled and ran `CreeperEngagementRulesTest` and `AlertConfigDefaultsTest` standalone with assertions enabled; both pass, including charged-creeper widening and the migration round trip. Full `javac` parse and flow analysis over `src/main/java` reports zero syntax or structure errors. Live verification that companions kill creepers without dying to them, and that an existing config has its injected creeper exclusion removed exactly once, remains required.

## 2026-08-04 (survival model: not dying without cowardice)

- Prompt/task: Companions need to attempt to not die, but also not be cowardly.
- Steps: Added `CompanionSurvivalProfile` (RECKLESS, DISCIPLINED default, CAUTIOUS) and a `combat` config block covering the withdraw health fraction, the profile, Second Wind, and the avenge duration. Wrote the pure `ResolveRules` encoding both halves as one boundary: a companion breaks off below its threshold, but five conditions veto that outright, namely the owner fighting within 12 blocks, avenging a downed owner, facing a creeper, being cornered, and the RECKLESS profile. Added `ResolveRulesTest` asserting each guarantee by name, plus profile behavior, re-engage hysteresis, and Second Wind gating, and registered `resolveRulesCheck`. Added `FightingWithdrawalGoal`, which holds MOVE and LOOK so the look control stays locked on the threat while the companion moves backwards, prefers falling back behind a healthy same-owner ally over open ground, re-evaluates the vetoes every tick so an owner joining the fight ends the withdrawal immediately, and ends on a re-engage threshold set a clear 25 percent above the break-off point so companions do not oscillate. Added server-side withdrawing, avenging, and Second Wind state to the entity with a `tickSurvivalState` driver, applied Second Wind as a 30 percent incoming damage reduction inside `hurt`, and hooked `LivingDeathEvent` so a player's death puts every owned companion within 32 blocks into the avenging state and points those without a target at the killer.
- Rationale: The two halves of the request conflict unless cowardice is defined precisely, so the definition is encoded rather than tuned. Cowardice is treated as two separate things: breaking off when the owner still needs you, which the vetoes prevent, and turning your back to run, which the goal prevents by retreating backwards under a locked look control. That split is what lets a companion be self-preserving and still never read as timid. Second Wind reduces incoming damage only and never increases damage dealt, which keeps it inside the plan's rule that decision quality may improve but combat numbers may not. Re-engage hysteresis exists because a symmetric threshold would make a wounded companion flicker in and out of melee on single hits.
- Build/Test: `gradlew check build` cannot run here; `maven.neoforged.net`, `maven.blamejared.com`, and `maven.theillusivec4.top` return HTTP 403 on CONNECT so `neoform-runtime` never resolves. Compiled and ran `ResolveRulesTest` standalone with assertions enabled and all cases pass, including each of the five guarantees asserted individually at near-zero health. Full `javac` parse and flow analysis over `src/main/java` reports zero syntax or structure errors. Live verification of withdrawal direction, ally-anchored fallback, avenging on owner death, and Second Wind timing remains required.

## 2026-08-04 (remove the taming system)

- Prompt/task: The whole taming system needs removed.
- Steps: Replaced the food-gate branch in `mobInteract` with a `recruit` method: the first interaction with an unowned companion tames it, plays the confirmation cue, stamps the first-recruited time, and applies the follow defaults. Added `CompanionRecruitMode` with INSTANT default and HANDSHAKE, which requires sneaking so structure residents can be spoken to without being taken along, and renamed the `taming` config block to `recruitment`. Audited what actually read the taming state before deleting it and found no compat provider touches it, contrary to the plan's assumption that Jade and WTHIT did, so the synced `FOOD1`, `FOOD2`, `FOOD1_AMT`, and `FOOD2_AMT` fields were removed outright rather than repurposed; only `FAVORITE_FOOD` remains. Deleted the `foodRequirements` map, `resourceRequirementResolved`, `untamedGreetingPlayed`, both `assignFoodRequirements` overloads, `syncFoodRequirements`, `getFoodRequirements`, `getFoodStatus`, `getWantedFoodsCompact`, and `foodRequirementComponent`, and repointed `getFoodStatusForGui` to a recruit hint for unowned companions. Stopped reading the legacy taming NBT keys so old saves drop them. Removed the four dead dialogue pools and all three `getRandomFoodRequirement` overloads from `CompanionData`, keeping `pickResource` for its existing progression-gating test coverage. Removed 125 orphaned dialogue and food-status translation keys, added the three new recruit strings, and added 24 config labels covering every option introduced by this overhaul.
- Rationale: Checking the actual readers first is what made the clean removal safe. The plan had proposed keeping the synced food fields alive as favourite-food display specifically to avoid breaking the Jade and WTHIT providers, but neither reads them, so preserving them would have left four synced fields and their serialization as permanent dead weight. Keeping HANDSHAKE as an option matters because structures spawn residents the player walks past constantly, and under INSTANT every one of those is a single misclick from joining the party.
- Build/Test: `gradlew check build` cannot run here; `maven.neoforged.net`, `maven.blamejared.com`, and `maven.theillusivec4.top` return HTTP 403 on CONNECT so `neoform-runtime` never resolves. Full `javac` parse and flow analysis over `src/main/java` reports zero syntax or structure errors, and a targeted search confirms no references remain to any removed member. All four language files were re-validated as JSON after editing. Live verification of first-interaction recruitment, HANDSHAKE mode, and loading a world containing previously untamed companions remains required.

## 2026-08-04 (squads, stances, and squad orders)

- Prompt/task: Interaction must be easy. Companions need to be separable by squad, squads must be mass-directable, and a squad directed toward a location must actually go there.
- Steps: Added `CompanionStance` (ESCORT, PATROL, WARD, HOLD) as a derived view over the existing follow/patrol/guard synced booleans, plus `setStance` as the single consistent mutator, and `CompanionStanceTest` asserting that all eight legacy boolean combinations resolve and that every stance round-trips losslessly. Added the `squad` package: `Squad` holding slot, name, colour, ordered membership and a standing order; `SquadOrder` as a record with FOLLOW, MOVE_TO, HOLD, WARD, and WORK types carrying an optional dimension-qualified destination; and `CompanionSquadData`, a `SavedData` persisted once on the overworld so squads are server-wide rather than per-level. Added `SquadService` as the only place membership and orders change, so the persisted roster and the stances of loaded members always move together. Added `SquadMoveToGoal` at its own priority band above every standing posture, which walks to the ordered destination, replans toward it on navigator completion or failure, falls back to pathing as far along the bearing as the navigator will accept so long marches progress in stages, and reports once rather than repeatedly when a route genuinely cannot be found. Added `CommandBatonItem` covering send-here, guard-here, add and remove companion, cycle squad, and cycle standing order without opening a screen, with a particle and sound marker at the destination. Added `/squad` and its `/sq` alias with list, add, clear, name, follow, hold, work, moveto, ward, allfollow, and allhold. Registered the baton item, model, recipe, and creative tab entry, persisted the companion's squad id, and added 30 translation keys. Corrected a priority collision I introduced mid-commit where the follow and delivery goals both landed on band 5; the movement bands are now one goal each from squad move order through idle behaviour.
- Rationale: Squad membership is stored in level data rather than on the entities because a squad has to survive its members dying, unloading, or being carried between dimensions, and because ordering a squad should not require every member to be loaded. The stance enum deliberately derives from the existing booleans instead of replacing them, which is a deviation from the plan's proposal to make the enum authoritative: the three booleans are read by every job goal, both HUD integrations, and the movement goals, and the actual defect was that scattered setters could produce contradictory combinations, which a single mutator fixes without touching any reader. Orders carry their dimension so a destination set in the Overworld does not send a squad walking after the player follows them into the Nether.
- Build/Test: `gradlew check build` cannot run here; `maven.neoforged.net`, `maven.blamejared.com`, and `maven.theillusivec4.top` return HTTP 403 on CONNECT, so `neoform-runtime` never resolves. Compiled and ran `CompanionStanceTest` standalone with assertions enabled and it passes. Full `javac` parse and flow analysis over `src/main/java` reports zero syntax or structure errors, and the language file was revalidated as JSON. Live verification of squad persistence across restart, long-distance move orders, and baton interactions remains required.

## 2026-08-04 (perimeter zones and the Perimeter Rod)

- Prompt/task: A dedicated stick-like tool that lets two points define the perimeter of a base, with an easy way to decide which companions guard it.
- Steps: Added the pure `ZoneMath` covering corner-order independence, vertical expansion clamped to world build limits, inclusive containment, signed edge distance, volume, and perimeter sampling, with `ZoneMathTest` and a registered `zoneGeometryCheck` task. Added `PerimeterZone`, which stores the two clicked corners plus a computed vertical span and can produce evenly spaced boundary sample points by walking the rectangle edge by edge, and `PerimeterZoneData`, a per-level `SavedData` with a volume cap and a per-player zone cap. Added `PerimeterRodItem`: the first click marks a corner, the second completes the zone, sneak-click restarts the corner, right-click air cycles between zones, sneak right-click air cancels a pending corner, and right-clicking a companion posts its whole squad to the selected zone. Posting distributes squad members across boundary sample points rather than stacking them at the centre. Added `zones.depthBelow` (default 12) and `zones.heightAbove` (default 24) config, item model, recipe, creative tab entry, and 18 translation keys.
- Rationale: The vertical extent is the part that decides whether a perimeter actually holds. Two clicked corners only give X and Z, and a box limited to those corners is bypassed by tunnelling underneath or flying over, so the zone expands downward and upward by default and clamps to the world build limits. Zones are stored per level rather than server-wide, unlike squads, because a zone's coordinates are meaningless outside the dimension it was drawn in. Distributing members around the boundary rather than clustering them is what converts several guards into actual coverage; the sampling is bounded so a very large base does not generate an unbounded number of posts.
- Build/Test: `gradlew check build` cannot run here; `maven.neoforged.net`, `maven.blamejared.com`, and `maven.theillusivec4.top` return HTTP 403 on CONNECT so `neoform-runtime` never resolves. Compiled and ran `ZoneMathTest` standalone with assertions enabled and it passes, including world-limit clamping and the sampling bounds. Full `javac` parse and flow analysis over `src/main/java` reports zero syntax or structure errors and the language file revalidates as JSON. Live verification of corner marking, zone persistence across restart, and squad posting remains required.

## 2026-08-04 (perception limits and ranged fire discipline)

- Prompt/task: Make companions offensively smart without making them omnipotent, and stop ranged classes shooting through the player.
- Steps: Added the pure `PerceptionRules` encoding the sensing model: a 120-degree forward cone with omnidirectional awareness inside six blocks, multiplicative reductions for darkness, rain, thunder, and the target sneaking, an intelligence multiplier hard-capped at plus or minus twenty percent, memory confidence that decays to zero over a configurable window, reduced confidence for relayed contacts, and a reaction delay clamped between three and ten ticks. Added the pure `FireLineRules` computing point-to-segment distance from an ally to a shot line, treating the shot as a segment rather than an infinite ray so allies behind the shooter or beyond the target do not block. Added `PerceptionRulesTest` and `FireLineRulesTest` with the boundary and degenerate cases, and registered `perceptionRulesCheck` and `fireLineCheck`. Added `CompanionFireDiscipline` as the live half, bounding its friendly search to the shot's own bounding box, and gated all four ranged attack paths on it: `ArcherRangedBowAttackGoal` returns and retries next tick, `ArbalistCrossbowAttackGoal` stays charged and waits, `MageRangedAttackGoal` defers the cast by one tick, and `FirearmAttackGoal` holds fire without consuming its cooldown. Added `combat.fireLaneClearance`, `combat.sightRange`, and `combat.memoryTicks` config with translations.
- Rationale: The sensing model is written as pure functions before any of it is wired into targeting because the constraint that matters most is the one that is easiest to lose later, namely that awareness must degrade and must stay capped. Making sneaking reduce what companions notice is deliberate: it keeps them fair to play alongside and preserves stealth as a real option rather than something the party ignores. Fire discipline is separated into pure geometry plus a thin live wrapper because the geometry is where the subtle bug lives, specifically treating the shot as an infinite line and thereby blocking on allies standing safely behind the shooter, which the segment clamp and its tests rule out. Each firing path holds in the way that suits its weapon rather than sharing one generic abort, so a charged crossbow stays charged and a firearm does not waste its cooldown.
- Build/Test: `gradlew check build` cannot run here; `maven.neoforged.net`, `maven.blamejared.com`, and `maven.theillusivec4.top` return HTTP 403 on CONNECT so `neoform-runtime` never resolves. Compiled and ran `PerceptionRulesTest` and `FireLineRulesTest` standalone with assertions enabled; both pass, including the intelligence caps across an extreme stat range, the reaction-delay clamp, and the behind-the-shooter and beyond-the-target segment cases. Full `javac` parse and flow analysis over `src/main/java` reports zero syntax or structure errors. Live verification that an archer holds fire with the player directly in line, and resumes once clear, remains required.

## 2026-08-04 (starter companion and overhaul documentation)

- Prompt/task: Start the player off with one good companion carrying a full iron setup, and bring the documentation in line with what was actually built.
- Steps: Added `StarterCompanionEvents` on `PlayerEvent.PlayerLoggedInEvent`, guarded by a once-per-player persistent flag, which spawns a Vanguard at a validated standing spot near the player, tames it, sets it to escort with alert enabled, fills it to full health, and enrols it in squad slot 1 so squad orders work from the first minute. Added `StarterCompanionGear` with IRON, LEATHER, and NONE, where IRON grants full iron armour, an iron sword, and a shield through `setManualEquipment` so the pieces are locked rather than being swapped out by the auto-equip scan. Gear is applied directly instead of through `CompanionData.getSpawnArmor`, whose roll yields nothing about forty percent of the time. Added `starter.enabled`, `starter.gear`, and `starter.grantTools` config, the latter also handing over the Command Baton and Perimeter Rod. Added `/companion starter` for worlds that predate the feature, with an operator-gated `force` variant. Removed two speculative helpers I had added to the starter class that nothing called. Added an implementation-status table to `PLAN_COMPANION_AI.md` recording which phases are done, partial, or untouched, plus the three places the implementation deliberately diverged from the spec. Wrote a player-facing `CHANGELOG.md` entry covering the whole 3.49 to 3.57 range.
- Rationale: Vanguard is the starter because it is the only archetype that already implements shield use, taunting, and projectile soak, which makes it the strongest early-game survivability pick without adding a single point of damage anywhere. Using `setManualEquipment` rather than plain slot assignment matters because the automatic equipment logic will otherwise reconsider the starter's gear against whatever it later picks up. Recording the deviations in the plan is the point of that section: the spec was written before the code was read closely, and two of its instructions were based on assumptions that turned out to be wrong, so leaving the spec unamended would mislead whoever picks this up next.
- Build/Test: `gradlew check build` cannot run in this environment; the network policy returns HTTP 403 on CONNECT to `maven.neoforged.net`, `maven.blamejared.com`, and `maven.theillusivec4.top`, so `net.neoforged:neoform-runtime:1.0.13` never resolves and compilation is never reached. Verified instead by compiling and running all eight pure rule suites standalone with assertions enabled, all passing: alert exclusion migration, teleport policy, creeper engagement, resolve, perception, fire line, stance, and zone geometry. Full `javac` parse and flow analysis over `src/main/java` reports zero syntax or structure errors across every commit in this series. A local `gradlew check build` and the in-world verification listed throughout `SUGGESTIONS.md` remain required before release.

## 2026-08-04 (perception-driven targeting and threat priority)

- Prompt/task: Finish the remaining gaps, beginning with wiring the perception rules into target acquisition so companions actually stop being omniscient.
- Steps: Added the pure `ThreatAssessment` with weights spaced far enough apart that category ordering is stable rather than emergent, scoring owner-attacker, imminence, self-attacker, wound state, zone intrusion, proximity, contact confidence, and a saturating discount for targets squadmates already hold, plus a `shouldSwitch` hysteresis margin. Added `ThreatAssessmentTest` stating each priority rule as a named assertion, and registered `threatAssessmentCheck`. Added `PerceivedContact`, which holds last known position and velocity, decays confidence, and can extrapolate a predicted position. Added `CompanionPerception`, which sweeps on a configurable interval phase-offset by entity id, applies the field-of-view cone, line of sight, and the per-target darkness and sneak terms, relays only first-hand sightings between squadmates so hearsay cannot self-perpetuate, caps the contact list at sixteen by evicting the least certain, and exposes `bestTarget` and `scoreOf`. Rewrote `AlertGoal` from a `NearestAttackableTargetGoal` subclass into a plain target goal that selects from contacts, applies an intelligence-scaled reaction delay before acting on a newly spotted target, requires a decisive score margin before abandoning a current fight, and drops its target once memory of it has decayed. The unsafe-target tag and the player-configured exclusion list are unchanged. Wired perception into the entity tick and added `combat.perceptionSweepInterval`.
- Rationale: Target selection was the last place omniscience lived. Everything else could be limited, but as long as acquisition used a bounding-box scan for the nearest valid entity, companions saw through walls and darkness and could never be snuck past. Routing acquisition exclusively through the contact list is what makes the sensing rules load-bearing rather than decorative. Relaying only first-hand sightings matters more than it looks: without that restriction, two companions relaying to each other would keep a contact alive forever and quietly restore the omniscience. The switch-hysteresis margin exists because a pure argmax over threat scores makes companions abandon a fight the moment anything marginally better wanders into range.
- Build/Test: `gradlew check build` cannot run here; `maven.neoforged.net`, `maven.blamejared.com`, and `maven.theillusivec4.top` return HTTP 403 on CONNECT so `neoform-runtime` never resolves. Compiled and ran `ThreatAssessmentTest` standalone with assertions enabled and it passes, including the saturating ally discount and the rule that an owner-attacker still wins even when two allies are already on it. Full `javac` parse and flow analysis over `src/main/java` reports zero syntax or structure errors. Live verification that a sneaking player goes unnoticed, that companions search a last known position rather than tracking through walls, and that a squad spreads across multiple targets remains required.

## 2026-08-04 (zone breach ledger and investigation)

- Prompt/task: Finish the perimeter work so a guarded base actually reports what gets past it.
- Steps: Added `ZoneBreachTracker`, a server-tick handler sweeping each player's zones once a second, capped at twenty-four entities per zone per sweep, and skipping zones whose centre chunk is unloaded so a reload cannot produce phantom breaches. For each hostile inside a zone it asks whether any guard posted to that zone already perceives it; if so the intruder is handled and no breach is recorded. Anything unperceived starts a timer, and once it exceeds the configurable grace period the owner receives a chat warning and an audible cue, rate-limited so a persistent breach does not spam. Added a home-field advantage that is deliberately not a target lock: the nearest idle guard is handed the intruder's position as an investigation cue. Added the cue itself to the companion as an expiring server-side position, plus `InvestigateGoal`, which walks there, looks around, and yields immediately if the companion actually perceives something on the way. Added `zones.alertSeconds` config and its translations. Re-shifted the movement priority bands to accommodate the new goal, which now run one goal per band from squad move order through idle behaviour.
- Rationale: A guarantee that nothing ever crosses a perimeter would require omniscient guards, which is exactly what the perception layer exists to prevent, so the honest version is near-total coverage plus an explicit report when it fails. The distinction that makes this work is that the tracker knows things the guards do not, and it is careful never to hand that knowledge over directly: guards receive a place to go and look, and still have to see the intruder themselves before they will fight it. That keeps the sensing rules intact while making a guarded base feel genuinely watched. Skipping unloaded zones matters because an unloaded chunk is not a zone where nothing is happening, it is a zone where nothing exists, and treating those the same would fire warnings on every login.
- Build/Test: `gradlew check build` cannot run here; `maven.neoforged.net`, `maven.blamejared.com`, and `maven.theillusivec4.top` return HTTP 403 on CONNECT so `neoform-runtime` never resolves. Full `javac` parse and flow analysis over `src/main/java` reports zero syntax or structure errors and the language file revalidates as JSON. Live verification that a mob spawning inside a walled base raises a warning after the grace period, that a guard investigates and then engages once it sees the intruder, and that no warnings fire on world load, remains required.

## 2026-08-04 (squad HUD and data-driven hunt targets)

- Prompt/task: Finish the remaining work, namely the squad readout and the last outstanding defect-register item.
- Steps: Rewrote `HuntGoal` eligibility (defect D-26) to be data-driven: any adult, untamed animal qualifies by default so modded livestock works without a code change, the same `hunter_allowed` and `hunter_denied` entity tags the Hunter profession already defines apply here too, an explicit deny always wins, and the legacy `huntMobs` config list is still honoured as an additive source so existing setups are unchanged. Added `SquadHudOverlay`, a client GUI layer registered above the vanilla hotbar showing each squad's label, strength, and average health, colour-coded so any badly wounded member turns the line red. Built it from a new synced `SQUAD_LABEL` string on the companion rather than a roster packet, so the HUD needs no networking at all; the trade-off is that it counts only loaded companions, which is also the only set whose health is meaningful. Hid it behind `starter.squadHud`, and suppressed it while the GUI is hidden, a screen is open, or the debug overlay is up. Updated the plan's status table to v3.60 and expanded the player-facing changelog with the awareness and base-defence sections.
- Rationale: Reusing the profession's existing tags rather than introducing a parallel set keeps one place for a pack author to configure what counts as huntable, and defaulting to every adult wild animal is what actually fixes the original defect, since a hardcoded list of six vanilla mobs can never cover modded content. Deriving the HUD from synced entity state was chosen specifically to avoid adding a roster sync packet for what is a cosmetic readout; entity data for nearby companions is already on the client, so grouping it costs nothing and cannot desynchronise.
- Build/Test: `gradlew check build` cannot run here; `maven.neoforged.net`, `maven.blamejared.com`, and `maven.theillusivec4.top` return HTTP 403 on CONNECT so `neoform-runtime` never resolves. All nine pure rule suites were compiled and run standalone with assertions enabled and all pass. Full `javac` parse and flow analysis over `src/main/java` reports zero syntax or structure errors. The HUD is the single piece least verifiable here: `RegisterGuiLayersEvent`, `VanillaGuiLayers.HOTBAR`, and the `LayeredDraw.Layer` signature were written from knowledge of the 1.21.1 NeoForge client API without a compiler to confirm them, so that file is the first thing to check if the local build fails.

## 2026-08-04 (CI build pipeline and the first real compile)

- Prompt/task: Provide a single downloadable mod file rather than a folder, with an easy link at the front.
- Steps: This environment cannot reach `maven.neoforged.net`, so no jar had ever been produced here. Added `.github/workflows/build.yml`, which builds on every push to `main` or a `claude/**` branch, runs the full `check` suite, and publishes the jar to a single rolling release under the fixed asset name `ModernCompanions.jar`, so one permanent link always serves the newest build. The jar is also attached to each run as a workflow artifact as a fallback. Rewrote the top of `README.md` with that link and complete install instructions, including the per-operating-system location of the `mods` folder. The first CI run then gave this work its first real compile, which surfaced exactly one error across all twelve commits: `ZoneBreachTracker` reassigned its `intruders` list before capturing it in a pruning lambda, so it was not effectively final. Fixed by computing the capped list once into a final variable, and replaced the O(n²) `noneMatch` prune with a `retainAll` against a set of present ids.
- Rationale: Moving the build to CI is the only way to produce a jar at all given the blocked maven, and it has a second benefit that turned out to matter more: it is the compiler this work never had. Every commit until now was verified by a `javac` parse and flow-analysis pass, which catches syntax and structure but cannot catch a semantic rule like effective finality. One error in roughly four thousand lines suggests the parse-only approach held up well, but it did not hold up completely, which is exactly why the build needed to move somewhere it could actually run. Notably `SquadHudOverlay`, flagged in the previous entry as the least verifiable file because its GUI layer registration was written without a compiler, compiled cleanly.
- Build/Test: The CI run at commit `8d12c07` reached `:compileJava` and failed with the single error above; every other source file compiled, and the remaining output was pre-existing `EventBusSubscriber.bus()` deprecation warnings unrelated to this work. This commit fixes that error. The local `javac` parse remains clean and all nine pure rule suites still pass. The next CI run is the verification that matters.
